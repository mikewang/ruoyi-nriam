...
public class ExcelReader {
    ...
    private static List<ExcelDataVO> parseExcel(Workbook workbook) {
        ...
        if (workbook.getNumberOfSheets() >= 1) {
            ...
            if (sheet == null ) {
                logger.warning("解析Excel失败，sheet读取为空！");
            }
            else {
                // 获取第一行数据
                int firstRowNum = sheet.getFirstRowNum();
                logger.warning("解析Excel的sheet，第一行" + String.valueOf(firstRowNum));
    ...
    private static List<ExcelDataVO> parseExcel2(Workbook workbook) {
        List<ExcelDataVO> resultDataList = new ArrayList<>();
        // 解析sheet
...
//           String  returnValue2 = df.format(Double.valueOf(returnValue));
...
//        }
    ...
    private static ExcelDataVO convertRowToData(Row row) {
        ...
        logger.warning("cell.getNumericCellValue() is " + name);

        resultData.setSkuName(name);
