  ...
  <div class="app-container">
    <el-row :gutter="20">
      ...
      <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
        <el-form-item label="年份" prop="productyear">
          <el-date-picker
            type="year"
            format="yyyy"
            value-format="yyyy"
            v-model="queryParams.productyear"
            placeholder="请输入"
            clearable
            size="small"
            style="width: 240px"
      ...
      <el-row :gutter="10" class="mb8">
        <el-col :span="1.5">
          <el-button
            ...
            size="mini"
            @click="handleAdd"
            v-hasPermi="['achieve:product:list']"
          >新增
          </el-button>
        </el-col>
      ...
      <el-table v-loading="loading" :data="achieveList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="名称" align="center" prop="productname" :show-overflow-tooltip="true">
        </el-table-column>
        <el-table-column label="入选年度" align="center" prop="productyear" width="160"/>

        <el-table-column label="合作单位" align="center" prop="corperationunit" />
...
<script>
import {listProduct} from "@/api/achieve/product";
import TeamData from "@/views/public/team-data";

export default {
  // 标准
  name: "achieve_product_index",
  ...
  methods: {
    ...
    /** 新增按钮操作 */
    handleAdd() {
      this.$router.push({path: '/achieve/product'});
    },

    handleUpdate(row) {
      const productid = row.productid
      console.log("edit product id is ", productid);
      this.$router.push({path: '/achieve/product/' + productid});
    },
