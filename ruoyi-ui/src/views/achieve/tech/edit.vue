<template>
  <div class="app-container">
    ...
    <el-row :gutter="20">
      <el-form v-loading="loading" ref="form" :model="form" :rules="rules" label-width="180px" :key="timer"
      >
        <template>
          <el-tag size="medium" type="info">农业部主推技术信息</el-tag>
          <el-row>
            <el-col :span="8">
              <el-form-item label="名称" prop="techname">
                <el-input v-bind:readonly="readonly.basic" v-model="form.techname" placeholder="请输入农业部主推技术名称"
                          :show-overflow-tooltip="true"/>
              </el-form-item>
            </el-col>

            <el-col :span="8">
              <el-form-item label="入选年度" prop="techyear">
                <el-date-picker v-bind:readonly="readonly.basic" v-model="form.techyear" type="year" placeholder="请选择日期"
                                format="yyyy"
                                value-format="yyyy"
          ...
          <el-row>
            <el-col :span="8">
              <el-form-item label="是否为第一技术委托单位" prop="iffirst">
                <dict-data :readonly="readonly.basic" :dict-type-name="DictTypeNameIffirst"
                           :selected-dict-value="form.iffirst" :data-options="IffirstOptions"
                           @changeDictValue="changeFormDictType" :key="form.iffirst"></dict-data>
              </el-form-item>
            </el-col>
          ...
          <el-row>
            <el-col :span="16">
              <el-form-item label="主推技术要点说明" prop="keypoint">
                <el-input v-bind:readonly="readonly.basic" v-model="form.keypoint" placeholder="" type="textarea"/>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="16">
              <el-form-item label="主推技术性能说明" prop="performance">
          ...
          <el-row>
            <el-col :span="16">
              <el-form-item label="《农业主导品种和主推技术》封面及相关证明页" prop="basicfileList1">
                <bas-doc :basdoc="basDocTechTuiguan" :readonly="readonly.basic" @changeFileList="changeBasicfileList1" :key="basDocTechTuiguan.relatedid" ></bas-doc>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="16">
              <el-form-item label="其它附件" prop="basicfileList2">
                <bas-doc :basdoc="basDocTechQita" :readonly="readonly.basic" @changeFileList="changeBasicfileList2" :key="basDocTechQita.relatedid" ></bas-doc>
              </el-form-item>
            </el-col>
          </el-row>
...
import {getTech, addTech, updateTech, deleteTech,confirmTech, getTechConfirm} from "@/api/achieve/tech";
...
export default {
  ...
  data() {
    return {
      ...
      DictTypeNameIffirst: "是否为第一技术委托单位",

      IffirstOptions: [{dictLabel: "是", dictValue:  true}, {dictLabel:  "否", dictValue:  false}],

      basDocTechTuiguan: {relatedid: -2, attachtotype: "农业部主推技术", doctype: "农业部主推技术"},
      basDocTechQita: {relatedid: -2, attachtotype: "农业部主推技术", doctype: "其它附件"},
      ...
      rules: {
        ...
        keypoint: [
          {required: true, message: "要点说明不能为空", trigger: "blur"}
        ],
        performance: [
  ...
  beforeCreate() {
    ...
    const techid = this.$route.params && this.$route.params.techid;
  ...
  created() {
    ...
    let techid = this.$route.params && this.$route.params.techid;
    if (techid === undefined || Number(techid) === 0) {
      techid = undefined;
    } else {
  ...
  methods: {
    ...
    getData(techid) {
      ...
      this.loading = true;
      console.log("loading is begin, techid is ", techid);
      ...
      if (techid === undefined) {
        ...
        getTech(techid).then(response => {
          ...
          data.techyear = data.techyear.toString();
          ...
          this_.basDocTechTuiguan.relatedid = this_.form.techid;
          this_.basDocTechQita.relatedid = this_.form.techid;

          // 获取 审核结果信息。
          if (this.form.status === this.AchieveStatus.BuTongGuo) {
            getTechConfirm(techid,this.AchieveStatus.BuTongGuo).then(response => {
              console.log("getTechConfirm is ", response);
    ...
    changeFormDictType(dict) {
      if (dict.type === this.DictTypeNameIffirst) {
        console.log("changeFormDictType is ", dict);
        if (dict) {
          this.form.iffirst = dict.id;
        } else {
          this.form.iffirst = undefined;
        }
      }
      else {
    ...
    changeBasicfileList1(filelist) {
      ...
      console.log("农业部主推技术 is ", filelist.length, this.form.docList.length);
    ...
    deleteForm() {
      ...
      const techid = this.form.techid;

      this.$confirm('是否确认删除"' + this.form.techname + '"的成果?', "警告", {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this_.form.docList === undefined || this_.form.docList.length === 0) {
            this.msgError("请上传农业部主推技术等");
            return;
          }

          if (this_.opcode === "add") {

            if (this_.form.techid === undefined) {
              addTech(this.form).then(response => {
                if (response.data === 0) {
                  this_.msgError("提交审核失败");
                  this_.form.techid = undefined;
                } else {
                  this.msgSuccess("提交审核成功");
                  this.form.techid = response.data;
            ...
            if (result === 1) {
              this_.$confirm('是否确认农业部主推技术审核 通过?', "警告", {
                confirmButtonText: "确定",
                cancelButtonText: "取消",
                type: "warning"
              }).then(function () {
                confirmTech(this_.form).then(response => {
              ...
              if (note !== null && note !== undefined && note.trim() !== '' ) {
                this_.$confirm('是否确认农业部主推技术审核 不通过?', "警告", {
                  confirmButtonText: "确定",
                  cancelButtonText: "取消",
                  type: "warning"
                }).then(function () {
