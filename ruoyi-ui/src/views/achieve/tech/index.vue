  ...
  <div class="app-container">
    <el-row :gutter="20">
      ...
      <el-table v-loading="loading" :data="achieveList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="名称" align="center" prop="techname" :show-overflow-tooltip="true">
        </el-table-column>
        <el-table-column label="入选年度" align="center" prop="techyear" width="100"/>

        <el-table-column label="是否为第一技术委托单位" align="center" prop="iffirst" width="100">
          <template slot-scope="scope">
            <span v-if="scope.row.iffirst === 1" :key="Math.random()">是</span>
            <span v-else :key="Math.random()">否</span>
          </template>
        </el-table-column>
...
<script>
import {listTech} from "@/api/achieve/tech";
import TeamData from "@/views/public/team-data";

export default {
  // 标准
  name: "achieve_tech_index",
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listTech(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
    ...
    /** 新增按钮操作 */
    handleAdd() {
      this.$router.push({path: '/achieve/tech'});
    },

    handleUpdate(row) {
      const techid = row.techid
      console.log("edit tech id is ", techid);
      this.$router.push({path: '/achieve/tech/' + techid});
    },
