  ...
  <div class="app-container">
    <el-row :gutter="20">
      ...
      <el-table v-loading="loading" :data="achieveList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="名称" align="center" prop="standardname" :show-overflow-tooltip="true">
        </el-table-column>

        <el-table-column label="类别" align="center" prop="standardtypelinktext" width="100"/>
        <el-table-column label="标准号" align="center" prop="standardcode" width="60"/>

        <el-table-column label="编制人" align="center" prop="authors" />

        <el-table-column label="颁布年度" align="center" prop="publishyear" width="60"/>
...
<script>
import {listStandard} from "@/api/achieve/standard";
import TeamData from "@/views/public/team-data";

export default {
  // 标准
  name: "achieve_standard_index",
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listStandard(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
    ...
    /** 新增按钮操作 */
    handleAdd() {
      this.$router.push({path: '/achieve/standard'});
    },

    handleUpdate(row) {
      const standardid = row.standardid
      console.log("edit standard id is ", standardid);
      this.$router.push({path: '/achieve/standard/' + standardid});
    },
