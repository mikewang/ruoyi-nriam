<template>
  <div class="app-container">
    ...
    <el-row :gutter="20">
      ...
      >
        <template>
          <el-tag size="medium" type="info">软件著作权信息</el-tag>
          <el-row>
            <el-col :span="8">
              <el-form-item label="名称" prop="softwarename">
                <el-input v-bind:readonly="readonly.basic" v-model="form.softwarename" placeholder="请输入软件著作权名称"
            ...
            <el-col :span="8">
              <el-form-item label="证书号" prop="sotfwarecode">
                <el-input v-bind:readonly="readonly.basic" v-model="form.sotfwarecode" placeholder="请输入证书号"
                          :show-overflow-tooltip="true"/>
              </el-form-item>
            </el-col>
          ...
          <el-row>
            ...
            <el-col :span="8">
              <el-form-item label="授权日期" prop="certifacatedate">
                <el-date-picker v-bind:readonly="readonly.basic" v-model="form.certifacatedate" type="date" placeholder="请选择日期"
                                format="yyyy-MM-dd"
                ...
                <el-row :gutter="10" class="mb8" v-bind:hidden="readonly.basic">
                  <el-col :span="1.5">
                    <el-button plain
                               ...
                               v-hasPermi="['achieve:software:list']"
                    >新增
                    </el-button>
                  </el-col>
                  <el-col :span="1.5">
          ...
          <el-row>
            ...
            <el-col :span="16">
              <el-form-item label="软件著作权登记证书" prop="basicfileList1">
                <bas-doc :basdoc="basDocSoftwareQuanwen" :readonly="readonly.basic" @changeFileList="changeBasicfileList1" :key="basDocSoftwareQuanwen.relatedid" ></bas-doc>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
            ...
            <el-col :span="16">
              <el-form-item label="其它附件" prop="basicfileList2">
                <bas-doc :basdoc="basDocSoftwareQita" :readonly="readonly.basic" @changeFileList="changeBasicfileList2" :key="basDocSoftwareQita.relatedid" ></bas-doc>
              </el-form-item>
            </el-col>
          </el-row>
...
import {getSoftware, addSoftware, updateSoftware, deleteSoftware,confirmSoftware, getSoftwareConfirm} from "@/api/achieve/software";
...
export default {
  ...
  data() {
    return {
      ...
      basDocSoftwareQuanwen: {relatedid: -2, attachtotype: "软件著作权", doctype: "软件著作权证书"},
      basDocSoftwareQita: {relatedid: -2, attachtotype: "软件著作权", doctype: "其它附件"},
      ...
      rules: {
        ...
        sotfwarecode: [
          {required: true, message: "软件著作权号不能为空", trigger: "blur"}
        ],
  ...
  beforeCreate() {
    ...
    const softwareid = this.$route.params && this.$route.params.softwareid;
  ...
  created() {
    ...
    let softwareid = this.$route.params && this.$route.params.softwareid;
    if (softwareid === undefined || Number(softwareid) === 0) {
  ...
  methods: {
    ...
    getData(softwareid) {
      ...
      this.loading = true;
      console.log("loading is begin, softwareid is ", softwareid);
      ...
      if (softwareid === undefined) {
        ...
        getSoftware(softwareid).then(response => {
          ...
          this_.basDocSoftwareQuanwen.relatedid = this_.form.softwareid;
          this_.basDocSoftwareQita.relatedid = this_.form.softwareid;

          // 获取 审核结果信息。
          if (this.form.status === this.AchieveStatus.BuTongGuo) {
            getSoftwareConfirm(softwareid,this.AchieveStatus.BuTongGuo).then(response => {
              console.log("getSoftwareConfirm is ", response);
    ...
    changeBasicfileList1(filelist) {
      ...
      console.log("软件著作权证书 is ", filelist.length, this.form.docList.length);
    ...
    deleteForm() {
      ...
      const softwareid = this.form.softwareid;

      this.$confirm('是否确认删除"' + this.form.softwarename + '"的成果?', "警告", {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this_.opcode === "add") {
            ...
            if (this_.form.softwareid === undefined) {
              addSoftware(this.form).then(response => {
                if (response.data === 0) {
                  this_.msgError("提交审核失败");
                  this_.form.softwareid = undefined;
                } else {
                  this.msgSuccess("提交审核成功");
                  this.form.softwareid = response.data;
            ...
            if (result === 1) {
              this_.$confirm('是否确认软件著作权审核 通过?', "警告", {
                confirmButtonText: "确定",
                cancelButtonText: "取消",
                type: "warning"
              }).then(function () {
                confirmSoftware(this_.form).then(response => {
              ...
              if (note !== null && note !== undefined && note.trim() !== '' ) {
                this_.$confirm('是否确认软件著作权审核 不通过?', "警告", {
                  confirmButtonText: "确定",
                  cancelButtonText: "取消",
                  type: "warning"
                }).then(function () {
