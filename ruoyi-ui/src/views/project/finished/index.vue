        ...
        >
          <template slot-scope="scope">
            <el-button
              ...
              v-hasPermi="['project:finished:list']"
            >查看
            </el-button>
          </template>
        </el-table-column>
...
<script>
import {listFinished} from "@/api/project/project";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listFinished(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
    ...
    handleFinished(row) {
      ...
      console.log("finished project id is ", projectid);
      this.$router.push({ path: '/project/finished/' + projectid });
    },
