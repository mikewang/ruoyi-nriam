        ...
        >
          <template slot-scope="scope">
            <el-button
              ...
              v-hasPermi="['project:toacceptanceconfirm:list']"
            >审核
            </el-button>
          </template>
        </el-table-column>
...
<script>
import {listToacceptanceconfirm} from "@/api/project/project";
import TeamData from "@/views/public/team-data";

export default {
  // 项目验收审核
  name: "project_toacceptanceconfirm_index",
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listToacceptanceconfirm(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
    ...
    handleToconfirm(row) {
      ...
      console.log("toacceptanceconfirm project id is ", projectid);
      this.$router.push({ path: '/project/toacceptanceconfirm/' + projectid });
    },
