        ...
        >
          <template slot-scope="scope">
            ...
            <el-button
              ...
              type="text"
              icon="el-icon-check"
              @click="handleToAcceptConfirm(scope.row)"
              v-hasPermi="['project:project:toAcceptConfirm']"
            >申请验收
            </el-button>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    /** 新增按钮操作 */
    handleAdd() {
      this.$router.push({ path: '/zaiyan/project/0' });
    },

    handleUpdate(row) {
      const projectid = row.projectid
      console.log("edit project id is ", projectid);
      this.$router.push({ path: '/zaiyan/project/' + projectid });
    },

    handleToAcceptConfirm(row) {
      const projectid = row.projectid
