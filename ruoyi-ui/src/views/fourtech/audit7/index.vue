        ...
        >
          <template slot-scope="scope">
            <el-button size="mini"
                       ...
                       v-hasPermi="['fourtech:audit7:list']"
            >审批
            </el-button>
          </template>
        </el-table-column>
...
<script>
import {listAudit7Fourtech} from "@/api/fourtech/fourtech";
...
export default {
  ...
  methods: {
    ...
    getContractList() {
      ...
      listAudit7Fourtech(this.addDateRange(this.queryParams2, this.dateRange)).then(response => {
    ...
    handleAuditFourtech(row) {
      ...
      const path = '/fourtech/audit7/' + row.fourtechid;
