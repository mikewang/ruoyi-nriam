        ...
        >
          <template slot-scope="scope">
            <el-button size="mini"
                       ...
                       v-hasPermi="['fourtech:audit5:list']"
            >审批
            </el-button>
          </template>
        </el-table-column>
...
<script>
import {listAudit5Fourtech} from "@/api/fourtech/fourtech";
...
export default {
  ...
  methods: {
    ...
    getContractList() {
      ...
      listAudit5Fourtech(this.addDateRange(this.queryParams2, this.dateRange)).then(response => {
    ...
    handleAuditFourtech(row) {
      ...
      const path = '/fourtech/audit5/' + row.fourtechid;
