        ...
        >
          <template slot-scope="scope">
            <el-button size="mini"
                       ...
                       v-hasPermi="['fourtech:audit4:list']"
            >审批
            </el-button>
          </template>
        </el-table-column>
...
<script>
import {listAudit4Fourtech} from "@/api/fourtech/fourtech";
...
export default {
  ...
  methods: {
    ...
    getContractList() {
      ...
      listAudit4Fourtech(this.addDateRange(this.queryParams2, this.dateRange)).then(response => {
    ...
    handleAuditFourtech(row) {
      ...
      const path = '/fourtech/audit4/' + row.fourtechid;
