...
<div>
  ...
  <el-select v-model="filterId" placeholder="请选择获奖级别" style="display:block;"
...
export default {
  name: "PrizeLevel",
  components: {},
  props:['selectedPrizeLevel'],
  data() {
    return {
      // 遮罩层
      loading: true,
      // 传入的参数
      filterId: this.selectedPrizeLevel === undefined ? undefined : this.selectedPrizeLevel.toString(),
      // 数据源
      options: [],
      list: []
    };
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      const dictType = "获奖级别";
      console.log("加载 " + dictType + " 组件 " + this.selectedPrizeLevel);
    ...
    clearValue() {
      this.$emit('changePrizeLevel',{id:undefined, value:undefined});
    },
    ...
    changeValue(value) {
      ...
      if (value) {
        ...
        this.$emit('changePrizeLevel',adict);
