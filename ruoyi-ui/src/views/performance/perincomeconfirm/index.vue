      ...
      <el-table v-loading="loading" :data="performanceList"  @selection-change="handleSelectionChange" >
        ...
        <el-table-column label="状态" align="center" prop="status" width="100">
          <template slot-scope="scope">
            <span v-if="scope.row.status==='已确认'">
               {{scope.row.status}}
            </span>
...
<script>
import {
  listIncomereport, updateIncomereport, addIncomereport, deleteIncomereport, confirmFundreport,
  listIncomeconfirm, confirmIncomereport
...
import {exportContract} from "@/api/logis/contract";


export default {
  name: "perincome_confirm_index",
  ...
  data() {
    return {
      ...
      queryRules: {
        ...
        teamid: [
          {required: false, message: "所属团队不能为空", trigger: "blur"}
        ]
      },
      //
      timer: '',
  ...
  methods: {
    ...
    getList() {
      ...
      listIncomeconfirm(this.queryParams).then(response => {
    ...
    handleRemove(row){
      const this_ = this;
      this.$confirm('是否确认删除 成果转化收入信息?', "警告", {
    ...
    handleConfirm(row) {
      const this_ = this;
      this.$confirm('是否确认已核对了编号为"' + this.ids + '"的成果转化收入信息?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function () {
        confirmIncomereport(this_.ids).then(() => {
