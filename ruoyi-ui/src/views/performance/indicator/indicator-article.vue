  ...
  <div class="app-container">
    <el-row>
      ...
      <el-table v-loading="loading" :data="articleList">
        ...
        <el-table-column label="著作类型" align="left" prop="articletype"/>
...
<script>
import {listIndicatorArticle, updateIndicatorArticle} from "@/api/performance/indicator";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listIndicatorArticle(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
    ...
    /** 修改按钮操作 */
    handleUpdate(row) {
      console.log("编辑著作", row);

      this.$confirm('是否确认修改 ' + row.articletype + ' 分数为 ' +  row.points +' ?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function () {
        return updateIndicatorArticle(row);
      }).then(() => {
        this.getList();
