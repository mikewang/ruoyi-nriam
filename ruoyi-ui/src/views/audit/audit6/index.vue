...
<script>
import {listAudit6Sheet} from "@/api/audit/audit";
import {listAudit6Contract} from "@/api/audit/contract";
...
export default {
  ...
  methods: {
    getSheetList() {
      ...
      listAudit6Sheet(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
    ...
    getContractList() {
      ...
      listAudit6Contract(this.addDateRange(this.queryParams2, this.dateRange)).then(response => {
    ...
    handleAuditSheet(row) {
      ...
      const path = '/audit/audit6/' + row.sheetid;
      console.log("path is " + path);
      this.$router.push({path: path});
    },
    handleAuditContract(row) {
      console.log("update row is  ", row);
      const path = '/contract/audit6/' + row.contractid;
      console.log("path is " + path);
      this.$router.push({path: path});
    }
  }
