        ...
        >
          <template slot-scope="scope">
            <el-button v-if="scope.row.btn_audit"
                       ...
                       v-hasPermi="['contract:toexecute:list']"
            >审批
            </el-button>
          </template>
        </el-table-column>
...
import {listToexecuteContract} from "@/api/audit/contract";

export default {
  name: "contract_toexecute_index",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getContractList() {
      ...
      listToexecuteContract(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
